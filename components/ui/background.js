...
export default function RandomCircle() {
  ...
  const generateCircles = useCallback(() => {
    const numCircles = Math.max(15, Math.min(otherColors.length * 3, 30));
    ...
    return Array.from({ length: numCircles }, (_, i) => {
      ...
      return {
        id: i,
        x: col * cellSize + Math.random() * cellSize * 0.8,
        y: row * cellSize + Math.random() * cellSize * 0.8,
  ...
  return (
    <div
      className="absolute inset-0 w-svw h-svh overflow-hidden transition-all ease-out duration-300"
